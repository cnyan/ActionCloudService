...
# 定义探测数组的长度（是否开始运动）
ACTION_DETECT_ACTION_BEGIN_SIZE = -10
# 定义探测数组的斜率阈值
ACTION_DETECT_K_START_VALUE = 8
ACTION_DETECT_K_END_VALUE = -8
...
def windows():
    print('下载文件测试')
    file = 'data.csv'
    if os.path.exists(r'src/test'):
        return send_from_directory(r'src/test', file)
    else:
        return 'error'
...
def action_greatRecognition():
    # print('请求时间 --- {}'.format((int(round(time.time() * 1000)))))
    try:
        dataMat = []
        ...
        # print('返回结果时间 --- {}'.format((int(round(time.time() * 1000)))))
...
def GreatDealWithDynamicData(dataMat):
    ...
    while count < len(dataMat):
        ...
        # 判断动作是否开始
        if (not isActionDetectStart) and isInterval(action_slope_queue, mode='start'):
            # print('检测到动作开始时间 --- {}'.format((int(round(time.time() * 1000)))))
        ...
        if isActionDetectStart:
            ...
            if len(second_action_data_queue) > ACTION_WINDOW_SIZE:
                isActionDetectStart = False  # 动作提取完毕
                # 动作窗口 action_window
                action_window = second_action_data_queue[:ACTION_WINDOW_SIZE]
...
# 通过等差数组，求截距的方式提取动作数组
def isInterval(array, mode='start'):
    len_array = len(array)
    if len_array != np.abs(ACTION_DETECT_ACTION_BEGIN_SIZE):
        return False

    half_len_array = int(len_array / 2)
    if mode == 'start':
        interval = 0
        for i in range(half_len_array):
            if array[half_len_array + i] - array[i] > ACTION_DETECT_K_START_VALUE:
                interval += 1
            if interval > (half_len_array / 2):
                # print('start')
                return True
        else:
            return False
        ...
        interval = 0
        for i in range(half_len_array):
            if array[half_len_array + i] - array[i] < ACTION_DETECT_K_END_VALUE:
                interval += 1
            if interval > (half_len_array / 2):
                # print('end')
                return True
...
for hmm_suffix in hmm_suffix_array:
    ...
    hmm_model_list.append(hmm_guss_model)
...
def predictByGussianHMM(seen, n_components):
    ...
    for hmm_guss_model in hmm_model_list:
    ...
    # print('第二次得分最大值：' + str(action_number) + str(np.max(score_list)))
